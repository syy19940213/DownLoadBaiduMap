...
## lat and lng change to Mercator
    ...
    if cE.__len__() != 0:
        ...
        while i>= 0 :
            if lat <= -LLBAND[i]:
                ...
                break
            i-=1
    x,y =converter(lng,lat, cE)
    return x,y

def converter(x,y, cE):
    xTemp = cE[0] + cE[1] *  abs(x)
    cC = abs(y) / cE[9]
    yTemp = cE[2] + cE[3] * cC + cE[4] * cC * cC + cE[5] * cC * cC * cC + cE[6] * cC * cC * cC * cC + cE[7] * cC * cC * cC * cC * cC + \
